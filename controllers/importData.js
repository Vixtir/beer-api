...
function categorySave(category){
...
function importCategory(categories){
  ...
  return Promise.all(_categories.map(categorySave));
}

function styleSave(style){
  return new Promise((resolve, reject) => {
    let _style = new styleModel();
    _style.save((err, style)=>{ resolve(style) });
  })
}

function importStyle(categories){
  const _categories = categories.slice();
  return Promise.all(_categories.map(categorySave));
}
